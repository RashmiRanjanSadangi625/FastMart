...
import { Avatar, AvatarGroup, Button, Card, CardHeader, Menu, MenuItem, TableSortLabel, Modal, Box, Typography, IconButton } from '@mui/material';
...
const OrdersTable = () => {
  ...
  const sortedOrders = [...(adminOrder?.orders || [])].sort((a, b) => {
        ...
        <Box sx={{ position: "absolute", top: "50%", left: "50%", transform: "translate(-50%, -50%)", width: 500, bgcolor: "background.paper", boxShadow: 24, p: 4, borderRadius: 2 }}>
          {selectedOrder && (
            <>
              ...
              <Typography><strong>TotalPrice:</strong> ₹{selectedOrder?.totalPrice}</Typography>
              ...
              {selectedOrder?.orderItems.map((orderItem) => (
                <Box key={orderItem?.product?._id} sx={{ display: "flex", alignItems: "center", gap: 2, mt: 1 }}>
                  ...
                  <Typography>{orderItem?.product?.title} - ₹{orderItem?.product?.price}</Typography>
                </Box>
              ))}
